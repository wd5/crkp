...
class FirstServRequest(models.Model):
    ...
    req_attachment7 = models.BooleanField(verbose_name = u'Копия действующего договора энергоснабжения со всеми приложениями и дополнительными соглашениями.', default=False, blank=True)
...
class ThirdServRequest(models.Model):
    ...
    req_attachment5 = models.BooleanField(verbose_name = u'Копия Устава (со всеми изменениями или последняя редакция), либо выкопировку из Устава, содержащую титульный лист, раздел общих положений, раздел, определяющий полномочия исполнительного органа управления.', default=False, blank=True)
    ...
    req_attachment9 = models.BooleanField(verbose_name = u'Копия действующего договора энергоснабжения со всеми приложениями и дополнительными соглашениями.', default=False, blank=True)
...
# типовая заявка на услугу для "Договор и технические условия на подключение к электрическим сетям" Юридические лица и индивидуальные предприниматели до 670 кВт (включительно)
class FourthServRequest(models.Model):
    ...
    req_attachment11 = models.BooleanField(verbose_name = u'Копия действующего договора энергоснабжения со всеми приложениями и дополнительными соглашениями.', default=False, blank=True)
...
# типовая заявка на услугу для "Договор и технические условия на подключение к электрическим сетям" Юридические лица и индивидуальные предприниматели свыше 670 кВт
class FifthServRequest(models.Model):
    ...
    req_attachment13 = models.BooleanField(verbose_name = u'Копия акта разграничения балансовой принадлежностей сетей и эксплуатационной ответственности сторон.', default=False, blank=True)
    # добавлено
    req_attachment14 = models.BooleanField(verbose_name = u'При присоединении к электрическим сетям, номинальный класс напряжения которых составляет 35кВ и выше - однолинейная схема электрических сетей заявителя, с указанием возможности резервирования от собственных источников энергоснабжения (включая резервирование для собственных нужд) и возможности переключения нагрузок (генерации) по внутренним сетям заявителя.', default=False, blank=True)
